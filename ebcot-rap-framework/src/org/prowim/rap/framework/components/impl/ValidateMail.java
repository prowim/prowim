...
/**
 * Tests a mail address of validation.
 * 
 * @author Maziar Khodaei
 * @version $Revision$
 * @since 2.0
 */
public class ValidateMail extends Text
{
     ...
     */
    public ValidateMail(Composite parent, final Constraint constraint)
    {
        ...
        if (constraint.isRequired() && this.getText().equals(""))
        {
            ...
            ValidateMail.this.setBackground(ColorManager.REQUIRED_FIELD);
        }
        ...
        this.addVerifyListener(new VerifyListener()
        {
            ...
            {
                verified.setErrorState(validator.checkMail(event.text));
                // If a value is required, then set the background color red, else set background text default.
                if (verified.hasError())
                    ValidateMail.this.setBackground(ColorManager.REQUIRED_FIELD);
                else
                    ValidateMail.this.setBackground(ColorManager.BACKGROUND_COLOR);
