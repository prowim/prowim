...
/**
 * You can only set here float numbers. All other keys will not accepted. <br/>
 ...
 * @version $Revision$
 */
public class FloatLimit extends Text
{
     ...
     */
    public FloatLimit(Composite parent, final Constraint constraint)
    {
        ...
        this.addVerifyListener(new VerifyListener()
        {
            ...
            public void verifyText(VerifyEvent event)
            {
                ...
                char[] chars = new char[currentStr.length()];
                currentStr.getChars(0, chars.length, chars, 0);
                ...
                verified.setErrorState(validator.checkFloat(currentStr));

                if (verified.hasError())
                    FloatLimit.this.setBackground(ColorManager.REQUIRED_FIELD);
                else
                    FloatLimit.this.setBackground(ColorManager.BACKGROUND_COLOR);
        ...
        this.addKeyListener(new KeyListener()
        {
            ...
            public void keyPressed(KeyEvent e)
            {
                ...
                if ( !('0' <= e.character && e.character <= '9' || e.character == '.' || e.keyCode == 8))
                {
                    e.doit = false;
                }

                verified.setErrorState(validator.checkFloat(getText()));
