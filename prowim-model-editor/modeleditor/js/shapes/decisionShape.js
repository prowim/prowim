 ...
 */
{
	...
	/**
	 * Constructor: decisionShape
	 * 
	 * Constructs a new decisionShape.
	 */
	function decisionShape(bounds, fill, stroke, strokewidth) {
		this.bounds = bounds;
		this.fill = fill;
		this.stroke = stroke;
	 ...
	 * Extends mxShape.
	 */
	decisionShape.prototype = new mxActor();
	 ...
	 */
	decisionShape.prototype.redrawPath = function(path, x, y, w, h) {
		path.moveTo(0.5 * w, 0 * h);
		path.lineTo(w, 0.5 * h);
		path.lineTo(0.75 * w, h);
		path.lineTo(0.25 * w, h);
		path.lineTo(0 * w, 0.5 * h);

		path.close();
	};

	mxCellRenderer.prototype.defaultShapes['decisionShape'] = decisionShape;
}
