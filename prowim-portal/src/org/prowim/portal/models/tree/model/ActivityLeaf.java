 ...
 * $HeadURL: https://repository.ebcot.info/wivu/trunk/prowim-portal/src/de/ebcot/prowim/portal/models/tree/model/ActivityLeaf.java $
...
/**
 * This is a model for activities. Each activity can have knowledges as its children and can be child of {@link ProcessLeaf}.
 * 
 * @author Maziar Khodaei
 * @version $Revision: 4915 $
 */
public class ActivityLeaf extends DefaultLeaf
{
    /** Image of model */
    private final Image imageKey = Resources.Frames.Activity.Images.ACTIVITY_IMAGE.getImage();

    /**
     * Constructor. Each activity model should has an unique ID and a name
     * 
     * @param activity To create this model
     * 
     */
    public ActivityLeaf(Activity activity)
    {
        super(activity.getID(), activity.getName());
        setImage(imageKey);
    }
     ...
     * Constructor. Each activity model should has an unique ID and a name
     * 
     * @param actID ID of Activity. can not be null.
     * @param actName Name of Activity. Can not be null.
     * 
     */
    public ActivityLeaf(String actID, String actName)
    {
        super(actID, actName);
        setImage(imageKey);
    }
