...
/**
 * Get data for output params to activity in adds these to model.
 * 
 * @author Maziar Khodaei
 * @version $Revision$
 */
public class OutputParamController
{
    /************ Variable *****************/
    ...
    private final List<Parameter> outputParamList = new ArrayList<Parameter>();
     ...
     */
    public OutputParamController(final String activityInstID)
    {

        this.productsList = MainController.getInstance().getActivityOutputProduct(activityInstID);

        Iterator<Product> productIt = productsList.iterator();
        while (productIt.hasNext())
        {
            Product product = productIt.next();
            this.outputParamList.addAll(product.getParameters().getItem());
    ...
    public Object[][] getTableModel()
    {
        ...
        try
        {
            rowsData = new Object[outputParamList.size()][sizeOfColumns];
            Iterator<Parameter> it = outputParamList.iterator();
            int row = 0, col = 0;
            while (it.hasNext())
            {
        ...
        {
            LOG.error("Error by getTableModel in outputParamController: ", e);
        }
        return rowsData;
    }
