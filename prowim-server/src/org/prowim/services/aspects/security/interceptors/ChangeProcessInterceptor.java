 ...
 * $HeadURL: https://repository.ebcot.info/wivu/trunk/prowim-server/src/de/ebcot/prowim/services/aspects/security/interceptors/ChangeProcessInterceptor.java $
...
public class ChangeProcessInterceptor implements SecurityInterceptor
{
     ...
     * {@inheritDoc}
     * 
     * @see org.prowim.services.aspects.security.interceptors.SecurityInterceptor#onMethodCall(javax.interceptor.InvocationContext)
     */
    @AroundInvoke
        ...
        if ( !intercept(context.getMethod().getName(), context.getMethod().getDeclaringClass().getSimpleName(), params))
        {
            ...
            if (context.getMethod().getName().equals("createObject"))
            {
                ...
                if (returnedObject != null)
                {
                    ...
                    {
                        // TODO ??? What to do if user ID is null?
                        String userID = DefaultSecurityManager.getInstance().getUserID(sessionCoontext.getCallerPrincipal().getName());
                        if (userID != null)
                        {
                            DefaultSecurityManager.getInstance().setUserCanModifyEntity((String) returnedObject, userID);
                        }
                    }
                }
     ...
     * {@inheritDoc}
     * 
     * @see org.prowim.services.aspects.security.interceptors.SecurityInterceptor#intercept(java.lang.String, java.lang.String, java.lang.Object[])
     */
