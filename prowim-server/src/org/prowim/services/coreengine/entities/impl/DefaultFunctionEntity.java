 ...
 * $LastChangedBy: wardi $
 * $HeadURL: https://repository.ebcot.info/wivu/trunk/prowim-server/src/de/ebcot/prowim/services/coreengine/entities/impl/DefaultFunctionEntity.java $
...
/**
 * Implements {@link FunctionEntity}.
 * 
 * @author Saad Wardi
 * @version $Revision: 4816 $
 */
@Stateless
public class DefaultFunctionEntity implements FunctionEntity
{
    private static final Logger LOG = Logger.getLogger(DefaultFunctionEntity.class);
     ...
     * {@inheritDoc}
     * 
     * @see org.prowim.services.coreengine.entities.FunctionEntity#getFunction(java.lang.String)
     */
    @Override
    public Function getFunction(String id) throws OntologyErrorException
    {
        Validate.notNull(id);
        final RecordSet records = processEngine.getFunction(id);
        ...
        if (records.getNoOfRecords() > 0)
        {
            ...
            Function function = DefaultDataObjectFactory.createFunction(id, name, createTime);
            function.setDescription(description);
            LOG.debug("Returns function :  " + id + "  " + name);
            return function;

        }
        else
        {
            throw new IllegalStateException("Could not create function with id =  " + id);
        }
    }
