 ...
 * $LastChangedBy: wardi $
 * $HeadURL: https://repository.ebcot.info/wivu/trunk/prowim-server/src/de/ebcot/prowim/services/coreengine/entities/impl/DefaultProductWayEntity.java $
...
/**
 * Impelements {@link ProductWayEntity}.
 * 
 * @author Saad Wardi
 * @version $Revision: 4816 $
 */
@Stateless
public class DefaultProductWayEntity implements ProductWayEntity
{
    private static final Logger LOG = Logger.getLogger(DefaultProductWayEntity.class);
     ...
     * {@inheritDoc}
     * 
     * @see org.prowim.services.coreengine.entities.ProductWayEntity#getProductWay(java.lang.String)
     */
    @Override
    public ProductWay getProductWay(String id) throws OntologyErrorException
    {
        Validate.notNull(id);
        final RecordSet records = processEngine.getProductWay(id);
        ...
        if (records.getNoOfRecords() > 0)
        {
            ...
            ProductWay productway = DefaultDataObjectFactory.createProductWay(id, name, createTime);
            productway.setDescription(description);
            LOG.debug("Returns product :  " + id + "  " + name);
            return productway;

        }
        else
        {
            throw new IllegalStateException("Could not create product with id =  " + id);
        }
    }
