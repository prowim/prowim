 ...
 * $LastChangedBy: wardi $
 * $HeadURL: https://repository.ebcot.info/wivu/trunk/prowim-server/src/de/ebcot/prowim/services/coreengine/entities/impl/DefaultWorkEntity.java $
...
/**
 * Implements {@link WorkEntity}.
 * 
 * @author Saad Wardi
 * @version $Revision: 4816 $
 */
@Stateless
public class DefaultWorkEntity implements WorkEntity
{
     ...
     * {@inheritDoc}
     * 
     * @see org.prowim.services.coreengine.entities.WorkEntity#getWork(java.lang.String)
     */
    @Override
    public Work getWork(String id) throws OntologyErrorException
    {
        Validate.notNull(id);
        final RecordSet records = processEngine.getWork(id);
        ...
        if (records.getNoOfRecords() > 0)
        {
            ...
            Work work = DefaultDataObjectFactory.createWork(id, name, createTime);
            work.setDescription(description);
            return work;
        }
        else
        {
            throw new IllegalStateException("Could not create work with id =  " + id);
        }
    }
